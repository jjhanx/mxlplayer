...
- **가로 줄·인쇄 전환**: `src/App.tsx` — 화면용 OSMD는 `renderSingleHorizontalStaffline` 과 기본 페이지(엔들리스) 중 하나입니다. 인쇄는 `getPrintableContentBoxMm`·`setCustomPageFormat`·`applyStandardPrintEngravingMode` 등(`src/print/configurePrintOsmd.ts`)을 거쳐 `.score-print-host` 에 그린 뒤 `window.print()` 합니다.
