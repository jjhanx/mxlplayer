...
- **인쇄**: 같은 문서 **`.score-print-host`** 에 OSMD를 다시 렌더합니다. **`@page` 여백(`PRINT_PAGE_MARGIN_MM`)** 과 같은 값으로 **실제 인쇄 가능 영역(mm)** 을 `setCustomPageFormat` 으로 넘겨, 레이아웃과 브라우저 인쇄 영역 불일치(오른쪽 마디선 잘림)를 줄였습니다. **화면 가로 한 줄 모드와 관계없이** 인쇄에서는 `renderSingleHorizontalStaffline: false` 로 **표준 다페이지**로 나가며 줄마다 음자리·조표·박자표는 OSMD 기본입니다. 브라우저 **인쇄 배율은 100%** 권장, 밀도는 **`PRINT_OSMD_ZOOM`** 과 간격 규칙으로 조정합니다. `@media print` 에서 OSMD가 넣는 **`div#osmdCanvasPage{n}` 직계마다** `break-after: page` 를 둠(내부 `svg` 는 직계가 아님). **긴 곡이 한 장으로만 나오는 문제**를 피하려면: (1) 인쇄 시 **호스트를 `position:absolute` 가 아니라 일반 배치(`static`)** 로 두어 Chrome 등에서 페이지 단편(fragmentation)이 막히지 않게 함(`App.css`), (2) OSMD `render()` 가 `container.offsetWidth` 로 폭을 잡으므로 **`sizePrintHostToContentBoxMm` 를 `load()` 이전에** 호출함(`App.tsx`).
...
- **가로 줄·인쇄 전환**: `src/App.tsx` — 화면용 OSMD는 `renderSingleHorizontalStaffline` 과 기본 페이지(엔들리스) 중 하나입니다. 인쇄는 먼저 **인쇄 호스트 크기(px)를 설정**한 뒤(`sizePrintHostToContentBoxMm`) `load()`, 그 다음 `applyStandardPrintEngravingMode` 등(`src/print/configurePrintOsmd.ts`)과 `render()`로 `.score-print-host` 에 그리고 `window.print()` 합니다. 인쇄 전용 레이아웃·페이지 분리는 **`App.css`** 의 `@media print` (`.printing-score`).
